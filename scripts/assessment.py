...
def evaluate_by_activity(
    ...
    if all_target_scores and all_other_scores:
        ...
        all_labels = np.array([1] * len(all_target_scores) + [0] * len(all_other_scores))
...
def print_results(results: dict):
    ...
    for activity, stats in sorted_activities:
        ...
        print(f"{activity:<25} {stats['mean_log_prob']:>12.4f} {stats['std_log_prob']:>12.4f} {stats['n_samples']:>12}{marker}")
...
def main():
    ...
    parser.add_argument("--config", type=str, default="configs/assessment.yaml", help="Config file")
    parser.add_argument("--list-activities", action="store_true", help="List available activities")
    ...
    exp_name = cfg.experiment.name or f"{cfg.data.label_type}_{cfg.data.target_activity}_{timestamp}"
    ...
    train_loader, test_loader, info = get_esk_dataloaders(
        ...
        test_activities=list(cfg.data.test_activities) if cfg.data.test_activities else None,
    ...
    print(f"Training samples ({cfg.data.target_activity} only): {info['n_train_samples']}")
    ...
    trainer = Trainer(
        ...
        scheduler=scheduler,
        device=str(device),
        checkpoint_dir=str(output_dir / "checkpoints") if cfg.output.save_checkpoints else None,
        use_wandb=use_wandb,
    )
