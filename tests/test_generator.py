...
def test_generate_programs():
    ...
    for program in programs:
        ...
        for part in parts:
            ...
            assert (
                0.5 <= value_float <= 1.0
            ), f"Value {value_float} out of range [0.5, 1.0]"
            assert (
                abs(round(value_float * 10) / 10 - value_float) < 1e-9
            ), f"Value {value_float} is not a multiple of 0.1"
...
def test_generate_programs_with_config():
    ...
    for program in programs:
        ...
        for part in parts:
            ...
            assert (
                0.1 <= value_float <= 0.5
            ), f"Value {value_float} out of range [0.1, 0.5]"
