...
def load_pose_file(pose_path: Path) -> tuple[np.ndarray, str]:
    ...
    for i, kp in enumerate(SMPL_KEYPOINTS):
        pose_data[:, i, 0] = df.iloc[:, df.columns.get_level_values("bodyparts") == kp].iloc[:, 0].values  # x
        pose_data[:, i, 1] = df.iloc[:, df.columns.get_level_values("bodyparts") == kp].iloc[:, 1].values  # y
        pose_data[:, i, 2] = df.iloc[:, df.columns.get_level_values("bodyparts") == kp].iloc[:, 2].values  # z
...
class ESKPoseDataset(Dataset):
    ...
    def __init__(
        ...
        # Load data
        self.segments, self.labels, self.activity_names, self.metadata = self._load_data()
    ...
    def _load_data(self) -> tuple[np.ndarray, np.ndarray, list[str], list[dict]]:
        ...
        for video_id in self.video_ids:
            ...
            for start in range(0, n_frames - self.seg_len + 1, self.seg_stride):
                ...
                segment_labels = frame_labels[start:end].mean(axis=0)  # Average over time
                
                # Get dominant activity for this segment
                dominant_activity = get_dominant_activity(segment_labels, self.label_names)
...
def get_unique_activities(esk_dir: Union[str, Path], label_type: LabelType = "activity") -> list[str]:
