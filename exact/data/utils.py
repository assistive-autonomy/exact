...
def generate_motion(
    ...
    obs_tensor = (
        ...
        if obs.ndim == 1
        else torch.from_numpy(obs).to(device)
    )
...
def generate_trajectory(
    ...
    all_obss = torch.empty(
        (total_steps, JOINT_POS_DIM), dtype=torch.float32, device=device
    )
    ...
    for motion in reward_fn.motions:
        ...
        joint_positions = torch.from_numpy(joint_positions).to(
