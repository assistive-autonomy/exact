"""ESK dataset utilities for SMPL pose-based tasks.

Provides loaders and DataLoader helpers shared across anomaly detection,
segmentation, and parser modules. All functions assume SMPL 24-joint poses
...
def load_esk_labels(label_path: Path) -> tuple[list[str], list[str], list]:
...
class ESKPoseDataset(Dataset):
    """ESK Pose Dataset for activity-based anomaly detection or segmentation."""
    ...
    def _get_label_names(self) -> list[str]:
        ...
        for video_id in self.video_ids:
            ...
            if label_path.exists():
                label_names, _, _ = load_esk_labels(label_path)
                return label_names
        return []
    ...
    def _load_data(self) -> tuple[np.ndarray, np.ndarray, list[str], list[dict]]:
        ...
        for video_id in self.video_ids:
            ...
            except Exception as e:  # keep minimal logging here
                print(f"Error loading {pose_path}: {e}")
                continue

            label_path = self.label_dir / f"{video_id}_labels.pickle"
            if label_path.exists():
                label_names, _, annotations = load_esk_labels(label_path)
            ...
            for start in range(0, n_frames - self.seg_len + 1, self.seg_stride):
                ...
                segment_labels = frame_labels[start:end].mean(axis=0)
                ...
                if self.split == "train":
                    if self.target_activity is None:
                        raise ValueError("target_activity must be specified for training")
...
def get_unique_activities(
    ...
    if label_files:
        label_names, _, _ = load_esk_labels(label_files[0])
        return label_names
    return []
